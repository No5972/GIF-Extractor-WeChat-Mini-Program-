(function(fig) {
    ...
    var disposeIndexedFrame = function(screenDescriptor, prev, cur, next) {
        ...
        var nextData = next.context._context.getImageData(0, 0, next.canvas.width, next.canvas.height);
        ...
        switch(disposal) {
            case GifDisposal.BACKGROUND:
                ...
                next.canvas.context._context.putImageData(nextData, 0, 0);
                break;
            case GifDisposal.PREVIOUS:
                var prevData = prev !== null ? prev.context._context.getImageData(0, 0, prev.canvas.width, prev.canvas.height) : null;
                ...
                next.context._context.putImageData(nextData, 0, 0);
                break;
    ...
    function blitIndexedFrame(screenDescriptor, frame) {
        ...
        // debugger;
      var canvasData = frame.context._context.getImageData(0, 0, frame.canvas.width, frame.canvas.height);

        // TODO: canvas image data
        ...
        frame.context._context.putImageData(canvasData, 0, 0);
    } 
    ...
    GifImage.prototype.renderFrames = function() {
        ...
        for(var i = 0; i < frames.length; i++) {
            ...
            frames[i].context = canvas.context;
        }
        ...
        for(var i = 0; i < frames.length; i++) {
            ...
            if(cur === null) {
                ...
                // next.context.clear(next.context.COLOR_BUFFER_BIT);
            } else {
    ...
    fig.load = function(args) {
      // var files = args.files;
      var base64BytesArray = args.base64BytesArray;
      offScreenCanvas = args.offScreenCanvas;
      var raw = args.raw || false;
      var oncomplete = args.oncomplete || function(){};
      var onerror = args.onerror || function(file, err){};

        // var gifs = new Array(files.length);
        var remaining = 1;
      // debugger;
        
      var gifReader = new fig.GifReader(base64BytesArray);
        var gifs = [];
